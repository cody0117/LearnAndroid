...
public class AuthActivity extends Activity
{
    ...
    public static boolean checkAppBeforeAuth(Context context, String s, boolean flag)
    {
        ...
        String s1 = (new StringBuilder("db-")).append(s).toString();
        intent.setData(Uri.parse((new StringBuilder()).append(s1).append("://1/connect").toString()));
        ...
        if (list.size() > 1)
        {
            ...
            {
                String _tmp = TAG;
                (new StringBuilder("There are multiple apps registered for the AuthActivity URI scheme (")).append(s1).append(").  Another app may be trying to  impersonate this app, so authentication will be disabled.");
                return false;
            }
        }
        ...
        {
            throw new IllegalStateException((new StringBuilder("There must be a ")).append(com/dropbox/client2/android/AuthActivity.getName()).append(" within your app's package registered for your URI scheme (").append(s1).append("). However, it appears that an activity in a different package is registered for that scheme instead. If you have multiple apps that all want to use the same accesstoken pair, designate one of them to do authentication and have the other apps launch it and then retrieve the token pair from it.").toString());
        } else
        {
            return true;
        }
