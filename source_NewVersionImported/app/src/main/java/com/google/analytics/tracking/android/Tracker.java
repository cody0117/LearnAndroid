...
public class Tracker
{
    ...
    private void internalSend(String s, Map map)
    {
        ...
        map.put("hitType", s);
        mModel.setAll(map, Boolean.valueOf(true));
        if (!tokensAvailable())
        {
    ...
    public void sendException(String s, Throwable throwable, boolean flag)
    {
        ...
        {
            s1 = mExceptionParser.getDescription(s, throwable);
        } else
        {
            try
            {
            ...
            s1 = "Unknown Exception";
        }
        GAUsage.getInstance().setDisableUsage(true);
        sendException(s1, flag);
        GAUsage.getInstance().setDisableUsage(false);
    }
    ...
    public void setCustomDimension(int i, String s)
    {
        if (i <= 0)
        {
            Log.w((new StringBuilder("index must be > 0, ignoring setCustomDimension call for ")).append(i).append(", ").append(s).toString());
            return;
        } else
        {
    ...
    public void setCustomDimensionsAndMetrics(Map map, Map map1)
    {
        ...
        {
            Integer integer1;
            for (Iterator iterator1 = map.keySet().iterator(); iterator1.hasNext(); setCustomDimension(integer1.intValue(), (String)map.get(integer1)))
            {
        ...
        {
            Integer integer;
            for (Iterator iterator = map1.keySet().iterator(); iterator.hasNext(); setCustomMetric(integer.intValue(), (Long)map1.get(integer)))
            {
