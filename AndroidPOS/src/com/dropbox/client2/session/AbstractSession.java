    ...
    implements Session
{
    ...
    public HttpClient getHttpClient()
    {
        ...
        HttpProtocolParams.setUserAgent(basichttpparams1, (new StringBuilder("OfficialDropboxJavaSDK/")).append(DropboxAPI.SDK_VERSION).toString());
    ...
    private class DBClientConnManager extends ThreadSafeClientConnManager
    {
        ...
        private class IdleConnectionCloserThread extends Thread
        {
            ...
            public static void ensureRunning(DBClientConnManager dbclientconnmanager, int i, int j)
            {
                ...
                if (thread == null)
                {
                    IdleConnectionCloserThread idleconnectioncloserthread = dbclientconnmanager. new IdleConnectionCloserThread(i, j);
                    thread = idleconnectioncloserthread;
                    idleconnectioncloserthread.start();
                }
                com/dropbox/client2/session/AbstractSession$IdleConnectionCloserThread;
                JVM INSTR monitorexit ;
                return;
