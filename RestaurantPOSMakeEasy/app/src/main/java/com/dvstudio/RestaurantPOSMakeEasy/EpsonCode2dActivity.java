...
public class EpsonCode2dActivity extends Activity implements OnClickListener, OnItemSelectedListener, StatusChangeEventListener, BatteryStatusChangeEventListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //check close port
        Print printer = EpsonPrintSampleActivity.getPrinter();
        if(printer == null){
            finish();
            return ;
        }
    ...
    public void onDestroy() {
        super.onDestroy();
        if(!exitActivity){
            EpsonPrintSampleActivity.closePrinter();
        }
    }
    ...
    private void printBarcode(){
        ...
        if(text.getText().toString().isEmpty()){
            com.dvstudio.RestaurantPOSMakeEasy.EpsonShowMsg.showError(R.string.errmsg_notext, this);
            return ;
        }
	...
	public void onStatusChangeEvent(final String deviceName, final int status) {
		runOnUiThread(new Runnable() {
			...
			public synchronized void run() {
				EpsonShowMsg.showStatusChangeEvent(deviceName, status, EpsonCode2dActivity.this);
			}
		});
	}
	...
	public void onBatteryStatusChangeEvent(final String deviceName, final int battery) {
		runOnUiThread(new Runnable() {
			@Override
			public synchronized void run() {
				EpsonShowMsg.showBatteryStatusChangeEvent(deviceName, battery, EpsonCode2dActivity.this);
			}
		});
	}
}
